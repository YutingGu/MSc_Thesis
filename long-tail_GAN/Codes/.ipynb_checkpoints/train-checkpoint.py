...
def train_GAN(h0_size, h1_size, h2_size, h3_size, NUM_EPOCH, NUM_SUB_EPOCHS, BATCH_SIZE, DISPLAY_ITER, LEARNING_RATE,
              to_restore, model_name, dataset, GANLAMBDA):
    DATA_DIR = dataset + '/'
    ...
    output_path = "chkpt/" + dataset_name + "_" + model_name + "_" + str(GANLAMBDA) + "_" + str(LEARNING_RATE) + "/"
    
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    
    d_loss_path = '../train_log/' + dataset_name + "_" + model_name + "_" + str(GANLAMBDA) + "_" + str(LEARNING_RATE) + "/"+ 'd_loss_log'
    g_loss_path = '../train_log/' + dataset_name + "_" + model_name + "_" + str(GANLAMBDA) + "_" + str(LEARNING_RATE) + "/"+ 'g_loss_log'
    scores_path = '../train_log/' + dataset_name + "_" + model_name + "_" + str(GANLAMBDA) + "_" + str(LEARNING_RATE) + "/"+ 'scores_log'
    

    if not os.path.exists(d_loss_path):
        os.makedirs(d_loss_path)
    if not os.path.exists(g_loss_path):
        os.makedirs(g_loss_path)
    ...
    SHOW2ID, IDs_present, NICHE_TAGS, ALL_TAGS, OTHER_TAGS = load_pop_niche_tags(show2id_path, item_list_path,
                                                                                 niche_tags_path, n_items)
    print('Done.')
    ...
    user_x_niche_vectors, user_x_popular_n_vectors = load_vectors(user_popular_data, user_niche_data, OVERLAP_COEFFS,
                                                                  ITEM_FEATURE_DICT, N)
    ...
    for i in range(NUM_EPOCH):
      ...
      for bnum, st_idx in enumerate(range(0, N, BATCH_SIZE)):
          ...
          for ii, user_idx in enumerate(idxlist[st_idx:end_idx]):
              ...
              curr_sampled_tags_bin, curr_sampled_tags = sample_from_generator_new(
                  ...
                  np.asarray(curr_generator_out)[ii, USER_TAGS_TO_SAMPLE[user_idx + uid_start_idx]],
      ...
      for j_disc in range(NUM_SUB_EPOCHS):
          ...
          for disc_batch_idx in indices:
              ...
              _, curr_d_loss = sess.run([d_trainer, d_loss_mean],
                                        feed_dict={generator_network.input_ph: X, x_popular_n_id: curr_x_popular_id_n,
                                                    x_popular_g_id: curr_x_popular_id_g, x_niche_id: curr_x_niche_id,
                                                    x_generated_id: curr_x_generated_id,
                                                    generated_tags: total_sampled_tags, sampled_cnt: total_sampled_cnt,
          ...
          d_loss_log.append([i, j_disc, curr_d_loss])
      ...
      for j_gen in range(NUM_SUB_EPOCHS):
          ...
          for gen_batch_idx in indices:
              ...
              _, curr_g_loss, curr_g_loss_term_1, curr_g_loss_term_2 = sess.run(
                  ...
                  feed_dict={generator_network.input_ph: X, x_popular_n_id: curr_x_popular_id_n,
                              x_popular_g_id: curr_x_popular_id_g, x_niche_id: curr_x_niche_id,
                              x_generated_id: curr_x_generated_id, generated_tags: total_sampled_tags,
                              ...
                              gen_lambda: curr_gen_lamda, keep_prob: np.sum(0.7).astype(np.float32)})
          ...
          g_loss_log.append([i, j_gen, curr_g_loss, curr_g_loss_term_1, curr_g_loss_term_2, anneal])
      ...
      print('global-epoch:', i, 'gen-epoch:', j_gen, 'Vad: NDCG:', np.mean(ndcg_vad), 'Recall@20:',
            np.mean(recall_at_20), 'Recall@50:', np.mean(recall_at_50), 'Num_users:', len(ndcg_vad),
            len(recall_at_20), len(recall_at_50))
      scores.append([i, j_gen, np.mean(ndcg_vad), np.mean(recall_at_20), np.mean(recall_at_50)])
      ...
      with open(os.path.join(d_loss_path,'d_loss_log_{}.txt'.format(i)), 'w') as f:
          f.write('global_epoch,discr_epoch,d_loss\n')
          for row in d_loss_log:
              f.write('%s,%s,%s\n' % (row[0], row[1], row[2]))

      with open(os.path.join(g_loss_path,'g_loss_log_{}.txt'.format(i)), 'w') as f:
          f.write('global_epoch,gen_epoch,g_loss,vae_loss,gan_loss,anneal\n')
          for row in g_loss_log:
              f.write('%s,%s,%s,%s,%s,%s\n' % (row[0], row[1], row[2], row[3], row[4], row[5]))
  
      with open(os.path.join(scores_path,'scores_{}.txt'.format(i)), 'w') as f:
          f.write('global_epoch,gen_epoch,ndcg,r@20,r@50\n')
          for row in scores:
...
train_GAN(h0_size, h1_size, h2_size, h3_size, NUM_EPOCH, NUM_SUB_EPOCHS, BATCH_SIZE, DISPLAY_ITER, LEARNING_RATE,
          to_restore, model_name, dataset, GANLAMBDA)