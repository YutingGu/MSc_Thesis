...
def test_GAN(h0_size, h1_size, h2_size, h3_size, NUM_EPOCH, NUM_SUB_EPOCHS, BATCH_SIZE, DISPLAY_ITER, LEARNING_RATE, to_restore, model_name, dataset, GANLAMBDA):
    ...
    output_path = "chkpt/" + dataset_name + "_" + model_name + "_" + str(GANLAMBDA) + "_" + str(LEARNING_RATE) + "/" + "model_" + str(NUM_EPOCH-1)
    
    results_path = '../results/' + dataset_name + "_" + model_name + "_" + str(GANLAMBDA) + "_" + str(LEARNING_RATE)
    ...
    np.savetxt(results_path + "/" + "pred_all_test.txt",pred_all_test)

    with open(os.path.join(results_path,'evaluations.txt'), 'w') as f:
        f.write(str(np.mean(n100_list)) + "," + str(np.mean(r20_list)) + "," + str(np.mean(r50_list)) + "\n")

    with open(os.path.join(results_path,'user_li.txt'), 'w') as f:
        for user in user_li[0]:
            f.write(str(user) + "\n")
    
    print("Finish evaluations.")
...
test_GAN(h0_size, h1_size, h2_size, h3_size, NUM_EPOCH, NUM_SUB_EPOCHS, BATCH_SIZE, DISPLAY_ITER, LEARNING_RATE, to_restore, model_name, dataset, GANLAMBDA)